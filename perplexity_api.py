"""
Perplexity AI API Integration Module

This module provides functionality to interact with the Perplexity AI service.
...
class PerplexityAPI:
    """Class for interacting with the Perplexity AI API"""
    
    def __init__(self, api_key=None, base_url="https://api.perplexity.ai"):
        """
        Initialize the PerplexityAPI client
        
        Args:
            api_key (str, optional): The API key for Perplexity AI. If None, it will look for PERPLEXITY_API_KEY env var.
            base_url (str, optional): The base URL for the Perplexity API.
        """
        self.api_key = api_key or os.environ.get("PERPLEXITY_API_KEY")
        self.base_url = base_url
        self.settings_file = "perplexity_settings.json"
    ...
    def _load_settings(self):
        ...
        try:
            ...
            logger.error(f"Error loading Perplexity settings: {str(e)}")
            return default_settings
    ...
    def _save_settings(self, settings):
        ...
        try:
            ...
            logger.error(f"Error saving Perplexity settings: {str(e)}")
            return False
    ...
    def update_settings(self, settings):
        ...
        try:
            ...
            logger.error(f"Error updating Perplexity settings: {str(e)}")
            return False
    
    def get_settings(self):
        """
        Get current Perplexity API settings
    ...
    def generate_response(self, user_message, user_id=None, username=None, conversation_id=None, metadata=None):
        ...
        if not self.is_enabled():
            return {
                "success": False,
                "response": None,
                "error": "Perplexity API is not enabled"
            }
        ...
        # Construct the message payload for Perplexity API
        payload = {
        ...
        # Store metadata locally for logging
    ...
    def _log_chat(self, user_message, ai_response, user_id=None, username=None, metadata=None):
        ...
        logger.info(f"Added new chat entry, history now has {len(settings['chat_history'])} entries")
